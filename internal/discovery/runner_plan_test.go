...
var _ = Describe("planning discovery execution", func() {
	...
	It("uses the full request as the singleflight identity", func() {
		first, err := runKey(Options{Profiles: []string{"one"}, Hosts: []string{"a.example.test"}})
		Expect(err).ToNot(HaveOccurred())
		same, err := runKey(Options{Profiles: []string{"one"}, Hosts: []string{"a.example.test", "a.example.test"}})
		Expect(err).ToNot(HaveOccurred())
		otherProfile, err := runKey(Options{Profiles: []string{"two"}, Hosts: []string{"a.example.test"}})
		Expect(err).ToNot(HaveOccurred())
		perEngine, err := runKey(Options{Profiles: []string{"one", "naabu=two"}, Hosts: []string{"a.example.test"}})
		Expect(err).ToNot(HaveOccurred())
		otherHost, err := runKey(Options{Profiles: []string{"one"}, Hosts: []string{"b.example.test"}})
		Expect(err).ToNot(HaveOccurred())

		Expect(first).To(Equal(same))
		Expect(first).ToNot(Equal(otherProfile))
		Expect(first).ToNot(Equal(perEngine))

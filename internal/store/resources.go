...
type ResourceOpts struct {
	...
	FirstSeen string   `json:"firstSeen,omitempty" flag:"first-seen" help:"Only resources first seen in this range (>=from,<=to)"`
	LastSeen  string   `json:"lastSeen,omitempty" flag:"last-seen" help:"Only resources last seen in this range (>=from,<=to)"`
	Sort      string   `json:"sort,omitempty" flag:"sort" help:"Sort by worst, name, type, account, region, first-seen, last-seen or findings" default:"worst"`
...
func (o ResourceOpts) Validate() error {
	switch o.Sort {
	case "", "worst", "name", "type", "account", "region", "first-seen", "last-seen", "findings":
	default:
	...
	if _, err := parseResourceTimeRange("first-seen", o.FirstSeen); err != nil {
		return err
	}
	if _, err := parseResourceTimeRange("last-seen", o.LastSeen); err != nil {
		return err
	}
	return nil
}
...
func (o ResourceOpts) Scope(db *gorm.DB) (*gorm.DB, error) {
	...
	for _, filter := range []struct {
		...
		{column: "first_seen", label: "first-seen", value: o.FirstSeen},
		{column: "last_seen", label: "last-seen", value: o.LastSeen},
	} {
		rangeValue, err := parseResourceTimeRange(filter.label, filter.value)
		if err != nil {
			return nil, err
		}
		if rangeValue.from != nil {
			db = db.Where(filter.column+" >= ?", *rangeValue.from)
		}
		if rangeValue.to != nil {
			operator := " <= ?"
			if rangeValue.toExclusive {
				operator = " < ?"
			}
			db = db.Where(filter.column+operator, *rangeValue.to)
		}
	}
...
func parseResourceTimeRange(label, raw string) (resourceTimeRange, error) {
	var parsed resourceTimeRange
	if raw == "" {
		return parsed, nil
	}
	for _, bound := range strings.Split(raw, ",") {
		...
		case strings.HasPrefix(bound, "<="):
			operator = "<="
		default:
			return resourceTimeRange{}, fmt.Errorf("%s range %q must use >=from and/or <=to", label, raw)
		}
		value, dateOnly, err := parseResourceRangeTime(strings.TrimSpace(strings.TrimPrefix(bound, operator)))
		if err != nil {
			return resourceTimeRange{}, fmt.Errorf("%s range %q: %w", label, raw, err)
		}
		if operator == ">=" {
			if parsed.from != nil {
				return resourceTimeRange{}, fmt.Errorf("%s range %q has more than one lower bound", label, raw)
			}
			parsed.from = &value
		} else {
			if parsed.to != nil {
				return resourceTimeRange{}, fmt.Errorf("%s range %q has more than one upper bound", label, raw)
			}
			if dateOnly {
				value = value.AddDate(0, 0, 1)
				parsed.toExclusive = true
			}
			parsed.to = &value
		}
	}
	if parsed.from != nil && parsed.to != nil {
		valid := !parsed.from.After(*parsed.to)
		if parsed.toExclusive {
			valid = parsed.from.Before(*parsed.to)
		}
		if !valid {
			return resourceTimeRange{}, fmt.Errorf("%s range %q starts after it ends", label, raw)
		}
	}
	return parsed, nil
}

func parseResourceRangeTime(raw string) (time.Time, bool, error) {
	if raw == "" {
		return time.Time{}, false, fmt.Errorf("range bound is empty")
	}
	...
	if err != nil {
		return time.Time{}, false, fmt.Errorf("%q is not YYYY-MM-DD or RFC3339", raw)
	}
